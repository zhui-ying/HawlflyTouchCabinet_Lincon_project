...
static u16  SPI_Flash_ReadID(void);  	    //读取FLASH ID
static u8	 SPI_Flash_ReadSR(void);        //读取状态寄存器 
static void SPI_FLASH_Write_Enable(void);  //写使能 
static void SPI_Flash_Erase_Sector(u32 Dst_Addr);//扇区擦除
...
void SPI_Flash_Init(void)	//初始化SPI
{
	SPIx_Init();//初始化SPI FLASH的IO口
